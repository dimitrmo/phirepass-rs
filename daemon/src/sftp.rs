...
use phirepass_common::protocol::sftp::{
    SFTPFileChunk, SFTPListItem, SFTPListItemAttributes, SFTPListItemKind,
};
                ...
                Some(cmd) = cmd_rx.recv() => {
                    match cmd {
                        ...
                        SFTPCommand::Download { path, filename, msg_id } => {
                            info!("sftp download command received for {path}/{filename}: {msg_id:?}");
...
async fn list_dir(sftp_session: &SftpSession, path: &str) -> anyhow::Result<SFTPListItem> {
    ...
    let mut root = SFTPListItem {
        ...
        attributes: SFTPListItemAttributes {
            size: attributes.size.map(|x| x as u64).unwrap_or(0),
        },
    };
    ...
    for entry in sftp_session.read_dir(path).await? {
        ...
        root.items.push(SFTPListItem {
            ...
            items: vec![],
            attributes: SFTPListItemAttributes {
                size: entry.metadata().size.map(|x| x as u64).unwrap_or(0),
            },
        });
    }
